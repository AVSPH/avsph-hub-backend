...
import { generateInvoiceSchema, generateBusinessInvoiceSchema, approveInvoiceSchema, addInvoiceAdjustmentSchema, bulkInvoiceActionSchema, } from "../../types/invoice.types.js";
...
export async function bulkInvoices(request, reply) {
    const invoices = request.server.mongo.db?.collection("invoices");
    const businesses = request.server.mongo.db?.collection("businesses");
    if (!invoices || !businesses) {
        return reply.status(500).send({ error: "Database not available" });
