...
function parseRange(raw: string | undefined, fallback: number, max: number): number {
...
export async function getBusinessAttendanceStats(
  request: FastifyRequest<{ Params: BusinessIdParams; Querystring: RangeDaysQuery }>,
  reply: FastifyReply,
) {
  ...
  if (!(await ensureBusinessAccess(request, reply, businessId))) return;
  ...
  return {
    ...
    daily: (facet?.daily ?? []).map((d: { _id: string; count: number; hours: number }) => ({
      date: d._id,
      count: d.count,
      hours: d.hours,
    })),
...
export async function getBusinessRecruitmentStats(
  ...
  const stageTypeByJob = new Map<string, Map<string, string>>();
  const jobMeta = new Map<string, { title: string; status: string }>();
  ...
  for (const job of jobs) {
    ...
    jobCounts.total += 1;
    if (job.status === "open") jobCounts.open += 1;
    else if (job.status === "draft") jobCounts.draft += 1;
    else if (job.status === "closed") jobCounts.closed += 1;
  }
  ...
  const perOpenJob = new Map<string, number>();
  ...
  for (const row of grouped) {
    ...
    if (stageType === "hired") applicantStats.hired += row.count;
    else if (stageType === "rejected") applicantStats.rejected += row.count;
    else applicantStats.active += row.count;
  ...
  return {
    ...
    conversionRate:
      applicantStats.total > 0 ? applicantStats.hired / applicantStats.total : 0,
    topOpenJobs,
    updatedAt: new Date().toISOString(),
  };
}
...
export async function getBusinessWorkforceStats(
  request: FastifyRequest<{ Params: BusinessIdParams }>,
  reply: FastifyReply,
) {
  const db = request.server.mongo.db;
  ...
  const pick = <T>(rows: { _id: string; count: number }[], keys: string[]) => {
    const out = {} as Record<string, number>;
    for (const k of keys) out[k] = 0;
    for (const r of rows) if (r._id in out) out[r._id] = r.count;
    return out as T;
  };

  const byStatus = pick<{ active: number; on_leave: number; terminated: number }>(
    facet?.byStatus ?? [],
    ["active", "on_leave", "terminated"],
  );
  const byEmploymentType = pick<Record<string, number>>(facet?.byType ?? [], [
    "full-time",
    "part-time",
    "contract",
  ]);
  ...
  const clientRows = (facet?.byClient ?? []) as {
    _id: string | null;
    count: number;
  }[];
  ...
  if (clientIds.length > 0) {
    ...
    for (const c of clientDocs) {
      clientNameById.set(String(c._id), c.name as string);
    }
  }
  const byClient = clientRows.map((r) => ({
    clientId: typeof r._id === "string" ? r._id : null,
    client:
      (typeof r._id === "string" && clientNameById.get(r._id)) || "Unassigned",
    count: r.count,
  }));
  ...
  return {
    ...
    byEmploymentType,
    byClient,
    hires: (facet?.hires ?? []).map((h: { _id: string; count: number }) => ({
      month: h._id,
      count: h.count,
    })),
...
export async function getBusinessPayrollTrend(
  request: FastifyRequest<{ Params: BusinessIdParams; Querystring: RangeMonthsQuery }>,
  reply: FastifyReply,
) {
  const db = request.server.mongo.db;
  ...
  const currencyMap = new Map<
    ...
    {
      currency: string;
      periods: { month: string; netPay: number; calculatedPay: number; count: number }[];
      breakdown: {
        regular: number;
        overtime: number;
      ...
      statutory: { sss: number; pagIbig: number; philHealth: number };
      totalNetPay: number;
