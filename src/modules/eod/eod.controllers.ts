  ...
  if (action === "approve" && affected.length) {
    ...
    for (const report of affected) {
      try {
        ...
        if (staffMember) {
          await invoiceOnEodApproval(
            db as any,
            { ...report, status: "reviewed", isApproved: true } as any,
            staffMember,
          );
        }
      } catch (err) {
        request.server.log.error(
          err,
          `[AUTO-INVOICE] Failed to update invoice after bulk EOD approve (eodId: ${report._id})`,
        );
      }
    }
  }
