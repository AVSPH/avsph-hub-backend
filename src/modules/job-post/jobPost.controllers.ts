...
async function countApplicantsForJob(request: FastifyRequest, jobId: string) {
...
async function withApplicantCount<T extends { _id: unknown }>(
...
export async function getAllJobPosts(
  ...
  return Promise.all(
    result.map((jobPost) => withApplicantCount(request, jobPost)),
  );
}
...
export async function createJobPost(
  ...
  const { businessId, title, overview, employmentType, status, stages } =
    parseResult.data;
...
export async function submitPublicApplication(
  ...
  const { createApplicantSchema } =
    await import("../../types/applicant.types.js");
  ...
  const { firstName, lastName, email, phone, resume, coverLetter } =
    parseResult.data;
