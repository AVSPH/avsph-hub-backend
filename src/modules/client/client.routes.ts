...
import {
  clientJsonSchema,
  createClientJsonSchema,
  updateClientJsonSchema,
...
const clientRoutes: FastifyPluginAsync = async (fastify) => {
  // List clients
  fastify.get<{
    Querystring: { businessId?: string; status?: string; isActive?: string };
  }>(
    "/clients",
    {
  ...
  fastify.get<{
    ...
    "/clients/:id/weekly-report",
    {
      preHandler: [fastify.authenticate],
      schema: {
        description:
          "Hours + pay report for a client's staff over a date range " +
