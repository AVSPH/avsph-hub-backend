...
export async function getClientWeeklyReport(
  ...
  const missingBillRateStaff: string[] = [];
  ...
  for (const member of members) {
    ...
    const hasBillRate =
      typeof rawBillRate === "number" &&
      Number.isFinite(rawBillRate) &&
      rawBillRate > 0;
    const billRateUsd = hasBillRate ? (rawBillRate as number) : 0;
...
type Db = NonNullable<
  FastifyRequest["server"]["mongo"]["db"]
>;

interface UsdContext {
  usdToPhp: number | null;
  usdAvailable: boolean;
  toUsd: (amount: number, currency: string) => Promise<number | null>;
}
 ...
 */
async function buildUsdContext(db: Db): Promise<UsdContext> {
  const usdToPhp = await getExchangeRateValue(db, "USD", "PHP");
  const usdAvailable = !!usdToPhp && usdToPhp > 0;
  const cache = new Map<string, number | null>();

  async function currencyToPhp(currency: string): Promise<number | null> {
    if (currency === "PHP") return 1;
...
function eodDateFilter(from?: string, to?: string): Record<string, unknown> {
  if (!from && !to) return {};
...
async function billMember(
  ...
  const hasBillRate =
    typeof member.billRateUsd === "number" &&
    Number.isFinite(member.billRateUsd) &&
    member.billRateUsd > 0;
  const billRate = hasBillRate ? (member.billRateUsd as number) : 0;
...
function resolveWindow(from?: string, to?: string): {
  ...
  let f = from;
  let t = to;
  if (f && !ISO_DATE_RE.test(f)) return { effectiveDate: "", error: "Invalid 'from' date" };
  if (t && !ISO_DATE_RE.test(t)) return { effectiveDate: "", error: "Invalid 'to' date" };
  if (f && t && f > t) [f, t] = [t, f];
...
export async function getBusinessClientAnalytics(
  request: FastifyRequest<{
  ...
  if (win.error) return reply.status(400).send({ error: win.error });
  ...
  const staffByClient = new Map<string, typeof assignedStaff>();
  for (const s of assignedStaff) {
    const cid = String(s.clientId);
    if (!staffByClient.has(cid)) staffByClient.set(cid, []);
  ...
  for (const client of clientDocs) {
    ...
    for (const member of members) {
      ...
      if (!b.hasBillRate) missingBillRateCount++;
    }

    totalStaff += members.length;
    totalHours += cHours;
    totalRevenueUsd += cRevenue;
    if (cPaidResolved) totalPaidUsd += cPaid;
...
export async function getClientAnalytics(
  ...
  for (const member of members) {
    ...
    if (b.paidUsd == null) allUsdResolved = false;
    else totalPaidUsd += b.paidUsd;
    if (!b.hasBillRate) missingBillRateCount++;
  }
