...
async function generateInvoicesForDateRange(
  ...
  for (const staffMember of staffMembers) {
    try {
      ...
      const totalHoursWorked = eodRecords.reduce(
      ...
      result.generated++;
      result.details.push(
        `Generated invoice for ${staffName}: ${periodStart} to ${periodEnd} (${totalHoursWorked}h, $${calculatedPay})`,
      );
    } catch (err) {
      result.errors++;
      const staffName = `${staffMember.firstName} ${staffMember.lastName}`;
...
async function recoverMissingInvoices(fastify: FastifyInstance): Promise<void> {
  const db = fastify.mongo.db;
  if (!db) {
  ...
  if (earliestStaff.length === 0) {
    fastify.log.info(
      "[STARTUP] No active hourly staff found — skipping invoice recovery",
    );
    return;
  }
  ...
  if (expectedPeriods.length === 0) {
    fastify.log.info(
      "[STARTUP] No completed pay periods found — skipping invoice recovery",
    );
    return;
  }
  ...
  for (const period of expectedPeriods) {
    try {
      const result = await generateInvoicesForDateRange(
        fastify,
        period.start,
        period.end,
      );
      ...
      if (result.generated > 0) {
        fastify.log.info(
          `[STARTUP] Period ${period.start} to ${period.end}: generated ${result.generated}, skipped ${result.skipped}`,
        );
      }
    } catch (err) {
      fastify.log.error(
        err,
        `[STARTUP] Failed to recover invoices for period ${period.start} to ${period.end}`,
      );
    }
  }
  ...
  if (totalGenerated > 0) {
    fastify.log.info(
      `[STARTUP] Invoice recovery complete: generated ${totalGenerated} missing draft invoices`,
    );
  } else {
    fastify.log.info(
      "[STARTUP] Invoice recovery complete: no missing invoices",
    );
  }
}
